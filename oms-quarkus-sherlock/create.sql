...
create table shipment_detail (quantity integer not null, unit_price float(53) not null, product_id bigint, shipment_details_id bigserial not null, shipment_id bigint, primary key (shipment_details_id));
create table shipment_shipment_detail (Shipment_shipment_id bigint not null, shipmentDetailList_shipment_details_id bigint not null unique);
...
alter table if exists shipment_shipment_detail add constraint FK2huka5uyyb38p3qkcpjabxou2 foreign key (shipmentDetailList_shipment_details_id) references shipment_detail;
alter table if exists shipment_shipment_detail add constraint FKqbocjtt9kvvfrcdbla61jgo82 foreign key (Shipment_shipment_id) references shipment;
